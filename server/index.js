...
// ── Trust Proxy for Vercel/Rate Limiters ─────────────────────────────────────
...
// Force the request to pause until MongoDB is ready upon waking from cold-start
app.use(async (req, res, next) => {
  try {
    await connectDB();
    next();
  } catch (err) {
    console.error('Database connection failed in blocking middleware:', err.message);
    return res.status(500).json({ success: false, message: 'Internal Server Error: Database Unavailable' });
  }
});
